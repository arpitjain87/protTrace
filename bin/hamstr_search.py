...
def hamstr_run():
	for dirs in glob.glob(hamstr + '/genome_dir/*'):
		...
		command = '%s/bin/hamstr.pl -central -sequence_file=%s -taxon=misc -hmmset=%s -strict -representative -checkCoorthologsRef -outpath=%s -hit_limit=10 -cleanup' %(hamstr, file_name, protein_id, output)
